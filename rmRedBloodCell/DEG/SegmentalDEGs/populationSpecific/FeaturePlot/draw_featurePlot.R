...
for(cid_ in c(0:7)){
	for(region in c("caput","corpus","cauda")){
		...
		if(length(gene)>0){
			pdf(paste0("SegmentalDEGTop10_FeaturePlot_",cid,"_",region,".pdf"),width=12,height=4*length(gene))
			#p <- VlnPlot(obj,features=gene,idents=cid_,assay =  "RNA",
			#	group.by="regions",pt.size=0,
			#	ncol = 5)
			#p <- DoHeatmap(obj,features = gene, assay = "RNA",slot="scale.data") + NoLegend()
			p = FeaturePlot(obj, features = gene, cols = mycolor,
					split.by = "regions", slot="scale.data",ncol=3)
			print(p)
			dev.off()
		}
	}
