...
#load clustering information based on the first run
cluster.df=read.table("../all//cell_cluster_ids.tsv",header=F,sep="\t")
cluster.filter=!(cluster.df[,2]=="5") #remove red blood cells
cells.selected=cluster.df[cluster.filter,1]
...
raw.data.merged.total=read.table("../../matrix/epididymis_umi_matrix.tsv",sep="\t",header=T)
print("before filtering")
print(dim(raw.data.merged.total))
genes=rownames(raw.data.merged.total)
genes.filter=grep("Hb[a-b]-",genes,invert=T) #remove globin gene contanmination
raw.data.merged=raw.data.merged.total[genes.filter,cells.selected]
print("after filtering:")
print(dim(raw.data.merged))
...
#output cell ids
clusters=object.integrated$seurat_clusters
